    ...
    server.tool(
        ...
        async ({ sheetId, include, pageSize, page, includeAll }) => {
            try {
                ...
                logger.error(`Failed to get discussions for sheet ID: ${sheetId}: ${error.message}`);
                return {
                    content: [
                        {
                            type: "text",
    ...
    server.tool(
        ...
        async ({ sheetId, rowId, include, pageSize, page, includeAll }) => {
            try {
                ...
                logger.error(`Failed to get discussions for row ID: ${rowId} in sheet ID: ${sheetId}: ${error.message}`);
                return {
                    content: [
                        {
                            type: "text",
    ...
    server.tool(
        ...
        async ({ sheetId, commentText }) => {
            try {
                ...
                logger.error(`Failed to create discussion on sheet ID: ${sheetId}: ${error.message}`);
                return {
                    content: [
                        {
                            type: "text",
    ...
    server.tool(
        ...
        async ({ sheetId, rowId, commentText }) => {
            try {
                ...
                logger.error(`Failed to create discussion on row ID: ${rowId} in sheet ID: ${sheetId}: ${error.message}`);
                return {
                    content: [
                        {
                            type: "text",
