...
import {
  createSmartsheetDirectAPI,
  SmartsheetDirectAPI,
} from "./smartsheet-direct-api";
...
console.info(
  `[Config] Delete operations are ${allowDeleteTools ? "enabled" : "disabled"}`,
);
    ...
    this.server.tool(
      ...
      async ({ sheetId, rowId, columnId, include, pageSize, page }) => {
        try {
          ...
          const history = await this.api.getCellHistory(
            sheetId,
            rowId,
            columnId,
            include,
      ...
      this.server.tool(
        ...
        async ({ sheetId, rowIds, ignoreRowsNotFound }) => {
          try {
            ...
            const result = await this.api.deleteRows(
              sheetId,
              rowIds,
              ignoreRowsNotFound,
            );
    ...
    this.server.tool(
      ...
      async ({ sheetId, destinationName, destinationFolderId }) => {
        try {
          ...
          const result = await this.api.copySheet(
            sheetId,
            destinationName,
            destinationFolderId,
          );
    ...
    this.server.tool(
      ...
      async ({ sheetId, include, pageSize, page, includeAll }) => {
        try {
          ...
          const discussions = await this.api.getSheetDiscussions(
            sheetId,
            include,
            pageSize,
            page,
    ...
    this.server.tool(
      ...
      async ({ sheetId, rowId, commentText }) => {
        try {
          ...
          const result = await this.api.createRowDiscussion(
            sheetId,
            rowId,
            commentText,
          );
    ...
    this.server.tool(
      ...
      async ({ workspaceId, folderName }) => {
        try {
          ...
          const folder = await this.api.createWorkspaceFolder(
            workspaceId,
            folderName,
          );
...
export default {
  fetch(request: Request, env: Env, ctx: ExecutionContext) {
    ...
    process.env.SMARTSHEET_API_KEY =
      request.headers.get("SMARTSHEET_API_KEY") ||
      process.env.SMARTSHEET_API_KEY;
    process.env.SMARTSHEET_ENDPOINT =
      request.headers.get("SMARTSHEET_ENDPOINT") ||
      process.env.SMARTSHEET_ENDPOINT;
