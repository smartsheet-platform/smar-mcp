    ...
    server.tool(
      ...
      async ({ sheetId, include, pageSize, page }) => {
        try {
          ...
          logger.error(`Failed to get sheet with ID: ${sheetId}: ${error.message}`);
          return {
            content: [
              {
                type: "text",
    ...
    server.tool(
      ...
      async ({ url, include, pageSize, page }) => {
        try {
          ...
          logger.error(`Failed to get sheet with URL: ${url}: ${error.message}`);
          return {
            content: [
              {
                type: "text",
    ...
    server.tool(
        ...
        async ({ sheetId }) => {
          try {
            ...
            logger.error(`Failed to get sheet version for sheet ID: ${sheetId}: ${error.message}`);
            return {
              content: [
                {
                  type: "text",
      ...
      server.tool(
        ...
        async ({ sheetId, rowId, columnId, include, pageSize, page }) => {
          try {
            ...
            logger.error(`Failed to get cell history for row ${rowId}, column ${columnId} in sheet ${sheetId}: ${error.message}`);
            return {
              content: [
                {
                  type: "text",
      ...
      server.tool(
        ...
        async ({ sheetId, rowId, include }) => {
          try {
            ...
            logger.error(`Failed to get row ${rowId} in sheet ${sheetId}: ${error.message}`);
            return {
              content: [
                {
                  type: "text",
      ...
      server.tool(
        ...
        async ({ sheetId, rows }) => {
          try {
            ...
            logger.error(`Failed to add ${rows.length} rows to sheet ${sheetId}: ${error.message}`);
            return {
              content: [
                {
                  type: "text",
      ...
      if (allowDeleteTools) {
        server.tool(
          ...
          async ({ sheetId, rowIds, ignoreRowsNotFound }) => {
            try {
              ...
              logger.error(`Failed to delete ${rowIds.length} rows from sheet ${sheetId}: ${error.message}`);
              return {
                content: [
                  {
                    type: "text",
        ...
        logger.warning("Delete operations are disabled. Set ALLOW_DELETE_TOOLS=true to enable them.");
      }
      ...
      server.tool(
        ...
        async ({ sheetId }) => {
          try {
            ...
            logger.error(`Failed to get location for sheet ${sheetId}: ${error.message}`);
            return {
              content: [
                {
                  type: "text",
      ...
      server.tool(
        ...
        async ({ sheetId, destinationName, destinationFolderId }) => {
          try {
            ...
            logger.error(`Failed to copy sheet ${sheetId} to "${destinationName}": ${error.message}`);
            return {
              content: [
                {
                  type: "text",
      ...
      server.tool(
        ...
        async ({ name, columns, folderId }) => {
          try {
            ...
            logger.error(`Failed to create sheet "${name}": ${error.message}`);
            return {
              content: [
                {
                  type: "text",
