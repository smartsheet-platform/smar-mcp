 ...
 * @export
 */
export type ShareReportRequest = Array<Share> | Share;

export function ShareReportRequestFromJSON(json: any): ShareReportRequest {
    return ShareReportRequestFromJSONTyped(json, false);
}

export function ShareReportRequestFromJSONTyped(json: any, ignoreDiscriminator: boolean): ShareReportRequest {
    if (json == null) {
        return json;
    }
    ...
    if (instanceOfShare(json)) {
        return ShareFromJSONTyped(json, true);
    }
    if (Array.isArray(json)) {
        if (json.every(item => typeof item === 'object')) {
            if (json.every(item => instanceOfShare(item))) {
                return json.map(value => ShareFromJSONTyped(value, true));
            }
        }
        return json;
    }
...
export function ShareReportRequestToJSON(json: any): any {
    return ShareReportRequestToJSONTyped(json, false);
}

export function ShareReportRequestToJSONTyped(value?: ShareReportRequest | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }
    ...
    if (instanceOfShare(value)) {
        return ShareToJSON(value as Share);
    }
    if (Array.isArray(value)) {
        if (value.every(item => typeof item === 'object')) {
            if (value.every(item => instanceOfShare(item))) {
                return value.map(value => ShareToJSON(value as Share));
            }
        }
        return value;
    }
