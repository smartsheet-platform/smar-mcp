 ...
 * @export
 */
export type AddGroupMembersRequest = Array<GroupMember> | GroupMember;

export function AddGroupMembersRequestFromJSON(json: any): AddGroupMembersRequest {
    return AddGroupMembersRequestFromJSONTyped(json, false);
}

export function AddGroupMembersRequestFromJSONTyped(json: any, ignoreDiscriminator: boolean): AddGroupMembersRequest {
    if (json == null) {
        return json;
    }
    ...
    if (instanceOfGroupMember(json)) {
        return GroupMemberFromJSONTyped(json, true);
    }
    if (Array.isArray(json)) {
        if (json.every(item => typeof item === 'object')) {
            if (json.every(item => instanceOfGroupMember(item))) {
                return json.map(value => GroupMemberFromJSONTyped(value, true));
            }
        }
        return json;
    }
...
export function AddGroupMembersRequestToJSON(json: any): any {
    return AddGroupMembersRequestToJSONTyped(json, false);
}

export function AddGroupMembersRequestToJSONTyped(value?: AddGroupMembersRequest | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }
    ...
    if (instanceOfGroupMember(value)) {
        return GroupMemberToJSON(value as GroupMember);
    }
    if (Array.isArray(value)) {
        if (value.every(item => typeof item === 'object')) {
            if (value.every(item => instanceOfGroupMember(item))) {
                return value.map(value => GroupMemberToJSON(value as GroupMember));
            }
        }
        return value;
    }
