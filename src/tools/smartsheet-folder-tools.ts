...
export function getFolderTools(server: McpServer, api: SmartsheetAPI) {
    ...
    server.tool(
        ...
        try {
            ...
            logger.error(`Failed to get folder with ID: ${folderId}: ${error.message}`);
            return {
            content: [
                {
                type: "text",
    ...
    server.tool(
        ...
        try {
            ...
            logger.error(`Failed to create folder in workspace with ID: ${folderId}: ${error.message}`);
            return {
            content: [
                {
                type: "text",
    ...
    server.tool(
        ...
        try {
            ...
            logger.error(`Failed to create folder in workspace with ID: ${workspaceId}: ${error.message}`);
            return {
            content: [
                {
                type: "text",
