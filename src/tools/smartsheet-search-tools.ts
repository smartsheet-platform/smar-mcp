...
export function getSearchTools(server: McpServer, api: SmartsheetAPI) {
    ...
    server.tool(
        ...
        try {
            ...
            logger.error(`Failed to search for sheets with query "${query}": ${error.message}: ${error.message}`);
            return {
            content: [
                {
                type: "text",
    ...
    server.tool(
        ...
        try {
            ...
            logger.error(`Failed to search in sheet ${sheetId} with query "${query}": ${error.message}: ${error.message}`);
            return {
            content: [
                {
                type: "text",
    ...
    server.tool(
        ...
        async ({ url, query }) => {
        try {
            logger.info(`Searching for sheet with URL: ${url} with query: ${query}`);
            ...
            logger.error(`Failed to search in sheet ${url} with query "${query}": ${error.message}: ${error.message}`);
            return {
            content: [
                {
                type: "text",
    ...
    server.tool(
        ...
        try {
            ...
            logger.error(`Failed to search in sheet ${sheetId}: ${error.message}: ${error.message}`);
            return {
            content: [
                {
                type: "text",
    ...
    server.tool(
        ...
        try {
            ...
            logger.error(`Failed to search in sheet ${url}: ${error.message}: ${error.message}`);
            return {
            content: [
                {
                type: "text",
    ...
    server.tool(
        ...
        try {
            ...
            logger.error(`Failed to search for folders with query: ${query}: ${error.message}`);
            return {
            content: [
                {
                type: "text",
    ...
    server.tool(
        ...
        try {
            ...
            logger.error(`Failed to search for workspaces with query: ${query}: ${error.message}`);
            return {
            content: [
                {
                type: "text",
    ...
    server.tool(
        ...
        try {
            ...
            logger.error(`Failed to search for reports with query: ${query}: ${error.message}`);
            return {
            content: [
                {
                type: "text",
    ...
    server.tool(
        ...
        try {
            ...
            logger.error(`Failed to search for dashboards with query: ${query}: ${error.message}`);
            return {
            content: [
                {
                type: "text",
